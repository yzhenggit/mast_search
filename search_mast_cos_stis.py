def save_mast_result(mast_table, tb_name='./mast_search_result', yes_print=True):
    ...
    mast_table.write(tb_name, overwrite=True)
    if yes_print == True:
        print("Found %d entries, save mast search result to %s\n\n"%(len(mast_table), tb_name))
...
def search_mast_cos_stis(gal_name, gal_ra, gal_dec, gal_dist_kpc,
    ...
    search_r_deg: search sightlines within this radius, in unit of degree
                  ...
                  use search_r_kpc and gal_dist_kpc options.
    ...
    if len(mast_table) == 0:
        ...
        import os
        if os.path.isdir('./mast_search_results') == False:
            os.mkdir('./mast_search_results')

        tb_name = './mast_search_results/MAST_%s_%s_%.1fdeg_%.1fkpc_%s.csv'%(intrument_name_edit, gal_name,
                                                     search_r_deg, search_r_kpc,
                                                     today)
        save_mast_result(mast_table, tb_name=tb_name, yes_print=yes_print)
